...
from
(select *,
	ROW_NUMBER() Over(order by StepNumber) as row_step,
	ROW_NUMBER() Over(partition by status order by StepNumber) as grp
from Groupings) as sub
group by Status, (row_step - grp)
...
from (
    ...
    from (
        select number + 1975 as yearvalue
        from master.dbo.spt_values
        where type = 'P' and number + 1975 <= year(getdate())
          and number + 1975 not in (
